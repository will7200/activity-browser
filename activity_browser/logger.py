...
class ABTimedFileHandler(TimedRotatingFileHandler):

    def __init__(self, suffix: str, *args, **kwargs):
        # set dir and create it if it doesn't exist yet
        dir_path = str(platformdirs.user_log_dir(appname="ActivityBrowser", appauthor="ActivityBrowser"))
        os.makedirs(dir_path, exist_ok=True)
        self.filepath = os.path.join(dir_path, "ab_logs.{}".format(suffix))

        super().__init__(self.filepath, *args, **kwargs)
...
def advanced_setup():
    ...
    if not BUNDLED:
        ...
        file_handler = ABTimedFileHandler("json")
        file_handler.addFilter(log_filter)
        file_handler.setLevel(LOG_LEVEL)
